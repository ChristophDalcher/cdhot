...
public class End_to_End_NEG_001_Test extends AHOTTest {
	...
	public void execute() {
		...
		// do not check captcha
		// expertconsulting.recaptchacheckbox.click();
		
		expertconsulting.submit.click();
		
		//Check that the security alert message box appears with the correct text 
		String text = expertconsulting.getBrowserFinder().getDriver().switchTo().alert().getText();
		Assert.assertEquals("Text stimmt nicht",  "Please confirm you’re not a robot.", text);
		expertconsulting.getBrowserFinder().getDriver().switchTo().alert().accept();
		
		//check that we are still on the contact-form AND NOT at the confirmation at the very end, 
		// telling that the request has been sent!
		expertconsulting.check();
	}
}
