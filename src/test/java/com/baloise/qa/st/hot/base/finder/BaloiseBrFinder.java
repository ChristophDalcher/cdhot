...
public class BaloiseBrFinder extends BrFinder {

  public BaloiseBrFinder(WebDriver driver, int timeoutInSeconds) {
  ...
  public void waitForAjaxDone(int seconds) {
    ...
    boolean done = false;
    while (!done) {
      done = isAjaxDone() && isJSDone();
      if (System.currentTimeMillis() > time + 1000 * seconds) {
        Logger logger = LoggerFactory.getLogger(BaloiseBrFinder.class);
